import { ai as realAi, z } from './genkit';
import {
  componentUpdateTool,
  dataModelUpdateTool,
  beginRenderingTool,
...
export function createGulfFlow(ai: Genkit) {
  return ai.defineFlow(
    ...
    async (request, streamingCallback) => {
      ...
      const genkitConversation: Message[] = request.conversation.map(
        (message) => {
          const content: Part[] = message.parts.map(
            (part: { text?: string; gulfEvent?: GulfEvent }) => {
              if (part.text) {
      ...
      try {
        ...
        logger.info('Calling ai.generateStream...');
            ...
            for (const toolRequest of chunk.toolRequests) {
              ...
              switch (toolRequest.name) {
                case 'componentUpdate':
                  gulfMessage = { componentUpdate: toolRequest.input };
                  break;
                case 'dataModelUpdate':
                  gulfMessage = { dataModelUpdate: toolRequest.input };
                  break;
                case 'beginRendering':
                  gulfMessage = { beginRendering: toolRequest.input };
                  break;
              }
...
export const gulfFlow = createGulfFlow(realAi);